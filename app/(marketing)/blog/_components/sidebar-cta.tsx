...
import { bookACallUrl } from "@/lib/utm";
...
export function SidebarCTA({ slug }: { slug?: string }) {
  return (
    <div className="space-y-6">
      ...
      <div className="rounded-2xl border border-brand-gold/30 bg-brand-cream/40 p-5">
        ...
        <Link
          href={bookACallUrl({ medium: "blog-sidebar", content: slug })}
          className="mt-2 flex w-full items-center justify-center gap-1.5 rounded-full border border-neutral-300 bg-white px-3 py-2.5 text-xs font-medium text-neutral-800 transition hover:border-neutral-400 hover:bg-neutral-50"
        >
        ...
        <div className="flex gap-2">
          {socials.map((s) => (
            <a
              ...
              aria-label={s.label}
              className="flex h-8 w-8 items-center justify-center rounded-md border border-neutral-200 text-neutral-500 transition hover:border-brand-gold/30 hover:bg-brand-cream hover:text-brand-gold-dark"
            >
              {s.icon}
            </a>
          ))}
